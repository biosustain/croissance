...
def remove_outliers(series, window=30, std=2):
    ...
    outliers = (
        abs(values - values.rolling(window=window, center=True).median())
        < values.rolling(window=window, center=True).std() * std
    )
    outlier_mask = outliers[window:-window].values

    outliers = pandas.Series(
        data=series.values[~outlier_mask], index=series.index[~outlier_mask]
    )
